			...
			for j in range(save_num):
				...
				for index, num in enumerate(err_indices):
					if num == 1:
						...
						'_index' + str(i) + '.png', 
						str(targets[index]), str(targests[index]))
						i += 1
...
#save the distilled targets
	print("saveing the distilled targets")
	distilled_targets = simple_prediction(X_train)
	with gzip.open('distilled_labels.gz', 'wb') as f:
		f.write(distilled_targests)
